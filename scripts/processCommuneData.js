...
const codeInseeIndex = header.findIndex(col => col.toLowerCase().includes('insee') || col.toLowerCase().includes('code'));
const nomCommuneIndex = header.findIndex(col => col.toLowerCase().includes('nom') || col.toLowerCase().includes('commune'));
const vacantIndex = header.findIndex(col => col.toLowerCase().includes('vacant') && col.includes('2'));

console.log('Column indices:', {
  codeInsee: codeInseeIndex,
  nomCommune: nomCommuneIndex,
  vacant: vacantIndex
});
...
// Generate TypeScript file with only vacancy data
...
console.log('Generated communeData.ts successfully with only vacancy data');